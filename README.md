...
- **SessionStart 注入**：会话启动时给主 agent 一条强制规则，要求所有子代理必须使用 `run_in_background:true`（后台子代理可被线 2 正常拦截）。
...
实际项目很少碰到线 1 死循环，以下是线 2 的示例：

> 1. 主代理执行工具时发现子代理死循环
> ![](https://mgear-image.oss-cn-shanghai.aliyuncs.com/image/other/20260625094536884.png)
> 
> 2. 子代理被 StopTask 停止
> ![](https://mgear-image.oss-cn-shanghai.aliyuncs.com/image/other/20260625094251953.png)
...
> 如果你之前按 `Lionad-Morotar/cc-break-dead-loop` 添加过 marketplace，请先执行 `/plugin marketplace remove lionad-morotar`，避免与本集市的其他插件（如 cc-flow）冲突。

> **本插件不会自动修改你的 `~/.claude/CLAUDE.md`**。Claude Code 的插件机制目前不支持安装时执行脚本或生命周期钩子，因此无法在安装阶段向你的全局 CLAUDE.md 追加规则。如果你希望“子代理必须后台运行”这类规则在所有会话中全局生效，请手动将它复制到 `~/.claude/CLAUDE.md`。本插件会通过 **SessionStart hook** 在每会话启动时注入相同规则，作为无需手动配置的替代方案。
...
```bash
/plugin update cc-break-dead-loop@lionad-morotar
```

## 卸载

```bash
/plugin uninstall cc-break-dead-loop@lionad-morotar
```
...
- 会话启动时注入子代理使用规则，要求主 agent 必须使用 `run_in_background:true`
