...
C4Container
  ...
  System_Boundary(plugin, "cc-break-dead-loop") {
    ...
    Container(src, "plugin/src/", "ES Module", "核心业务逻辑")
...
graph TB
    ...
    subgraph "src/cli/"
        CLI[cli/index.mjs<br/>CLI 入口]
        CMD[commands/<br/>install / uninstall / status]
        UTL2[utils/<br/>config / fs / paths]
    end
...
| Hook 类型 | 职责 | 时序 |
|-----------|------|------|
| PostToolUse | **观察** Read 结果，检测 "Wasted call" / `file_unchanged` | Read 执行**后** |
...
Layer 4: plugin/src/handlers.mjs
    └── 参数缺失 → 静默跳过
    └── 状态不存在 → 放行
```
...
阻断时同时使用两种机制：
- **`permissionDecision: 'deny'`**：对主 agent 强制阻断 Read 操作
- **`additionalContext`**：对所有 agent 类型（含 subagent/teammate）注入停止引导文案

两者同时返回，覆盖已知 subagent 无视 deny 的 bug（#25000, #34692）。
...
| D6 | 三层 toolResponse 检测：字符串 / `file_unchanged` 对象 / `content` 对象 / `JSON.stringify` 兜底 | CC 的 toolResponse 格式可能因版本变化，`file_unchanged` 为实际返回格式 | 稍增加性能开销，但兼容性优先 |
...
### plugin/src/handlers.mjs — 双 Handler
...
- **输出**：`{ continue, suppressOutput }` 或 `{ hookSpecificOutput: { permissionDecision, additionalContext } }`

### plugin/src/state.mjs — 状态管理
...
### src/cli/ — CLI 工具
- **职责**：提供 `npx cc-break-dead-loop` 命令行工具
- **命令**：`install`（安装）、`uninstall`（卸载）、`status`（状态查询）
- **安装机制**：复制 `plugin/` 到 marketplace 目录，注册到 Claude Code 配置
