...
### 硬编码阈值
- Issue: `WARN_THRESHOLD = 3` 和 `BLOCK_THRESHOLD = 5` 在 `src/config.mjs` 中硬编码，无法根据工作流调整
- Files: `src/config.mjs`, `src/handlers.mjs`
...
- Files: `src/state.mjs`（`incrementCounter` 函数）
...
- Files: `src/handlers.mjs`（`preToolUseRead`）
...
- Files: `src/utils.mjs`（`sanitizeName`）, `src/state.mjs`（`getStateDir`）
...
- Files: `src/handlers.mjs`, `src/state.mjs`
...
- Files: `src/utils.mjs`（`getProjectName`）
...
- Files: `src/handlers.mjs`（`isWastedCall` 函数）
...
- Files: `src/subagentTranscriptReader.mjs`、`src/watcher.mjs`（`findAllAgentJsonls`、`parseAgentFromPath`）
...
### watcher 进程泄漏风险
- Files: `src/watcherLifecycle.mjs`（`ensureWatcherRunning`）
