...
- Why fragile: 死循环检测完全依赖 "Wasted call — file unchanged since your last Read" 这一文案。若 Claude Code 未来版本变更文案，检测将完全失效
- Safe modification: 修改检测逻辑时需保持向后兼容，新增文案模式而非替换
- Test coverage: `tests/handlers.test.mjs` 覆盖了字符串、对象 content、JSON.stringify 三种模式，但未覆盖 CC 文案变更场景

### 双重错误边界的重复逻辑
- Files: `src/index.mjs`, `plugin/scripts/node-runner.mjs`
